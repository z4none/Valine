...
ValineFactory.prototype._init = function () {
  ...
  try {
    ...
    root.config.meta = (
      root.config.guest_info ||
      root.config.meta ||
      META
    ).filter((item) => META.indexOf(item) > -1);
    let inputEl = (root.config.meta.length == 0 ? META : root.config.meta).map(
      (item) => {
        let _t = item == "mail" ? "email" : "text";
        return META.indexOf(item) > -1
          ? `<input name="${item}" placeholder="${root.locale["head"][item]}" class="v${item} vinput" type="${_t}">`
          : "";
      }
    );
    root.placeholder = root.config.placeholder || "Just Go Go";

    root.el.innerHTML = `<div class="vwrap"><div class="${`vheader item${inputEl.length}`}">${inputEl.join(
      ""
    )}</div><div class="vedit"><textarea id="veditor" class="veditor vinput" placeholder="${
      root.placeholder
    }"></textarea><div class="vctrl"><span class="vemoji-btn">${
      root.locale["ctrl"]["emoji"]
    }</span> | <span class="vpreview-btn">${
      root.locale["ctrl"]["preview"]
    ...
    root.alert = {
      ...
      show(o) {
        ...
        let _cBtn = `<button class="vcancel vbtn">${
          (o && o.ctxt) || root.locale["ctrl"]["cancel"]
        }</button>`;
        let _oBtn = `<button class="vsure vbtn">${
          (o && o.otxt) || root.locale["ctrl"]["sure"]
        }</button>`;
      ...
      query
        ...
        .then((ret) => {
          if (ret.length > 0) {
            ...
            v.save()
              .then((rt) => {
                Utils.find(lv, ".leancloud-visitors-count").innerText = rt.get(
                  "time"
                );
              })
              .catch((ex) => {
        ...
        .then((ret) => {
          if (ret.length > 0) {
            Utils.each(ret, (idx, item) => {
              ...
              let els = Utils.findAll(
                document,
                `.leancloud_visitors[id="${url}"],.leancloud-visitors[id="${url}"]`
              );
              Utils.each(els, (idx, el) => {
                let cel = Utils.find(el, COUNT_CONTAINER_REF);
...
ValineFactory.prototype.ErrorHandler = function (ex) {
  ...
  if ({}.toString.call(ex) === "[object Error]") {
    ...
    if (code == 101) root.nodata.show();
      ...
      (root.el &&
        root.nodata.show(
          `<pre style="text-align:left;">Code ${code}: ${msg}</pre>`
        )) ||
        (console && console.error(`Code ${code}: ${msg}`));
  } else {
    (root.el &&
      root.nodata.show(
        `<pre style="text-align:left;">${JSON.stringify(ex)}</pre>`
      )) ||
...
ValineFactory.prototype.bind = function (option) {
  ...
  let xssFilter = (content) => {
    ...
    let __replaceVal = (node, attr) => {
      ...
      val && Utils.attr(node, attr, val.replace(/(javascript|eval)/gi, ""));
    };
    Utils.each(ns, (idx, n) => {
      if (n.nodeType !== 1) return;
      if (rejectNodes.indexOf(n.nodeName) > -1) {
        // console.log(n.nodeName)
        if (n.nodeName === "INPUT" && Utils.attr(n, "type") === "checkbox")
          Utils.attr(n, "disabled", "disabled");
        else Utils.remove(n);
      }
  ...
  for (let i in mapping) {
    if (mapping.hasOwnProperty(i)) {
      ...
      inputs[_v] = _el;
      _el &&
        Utils.on("input change blur", _el, (e) => {
          if (_v === "comment") syncContentEvt(_el);
          else
            defaultComment[_v] = Utils.escape(
              _el.value.replace(/(^\s*)|(\s*$)/g, "")
            );
        });
    }
  }
  ...
  let query = (no = 1) => {
    ...
    cq.find()
      .then((rets) => {
        ...
        _vpage.innerHTML =
          size * no < count
            ? `<button type="button" class="vmore vbtn">${root.locale["ctrl"]["more"]}</button>`
            : "";
    ...
    .then((num) => {
      if (num > 0) {
        ...
        Utils.find(
          root.el,
          ".vcount"
        ).innerHTML = `<span class="vnum">${num}</span> ${root.locale["tips"]["comments"]}`;
        query();
      } else {
        root.loading.hide();
      }
  ...
  let insertDom = (rt, node, mt) => {
    ...
    let _img = _avatarSetting["hide"]
      ? ""
      : `<img class="vimg" src="${
          _avatarSetting["cdn"] + md5(rt.get("mail")) + _avatarSetting["params"]
        }">`;
    let ua = rt.get("ua") || "";
    let uaMeta = "";
    if (ua) {
    ...
    let _t = rt.get("link") || "";
    _nick = _t
      ? `<a class="vnick" rel="nofollow" href="${_t}" target="_blank" >${rt.get(
          "nick"
        )}</a>`
      : `<span class="vnick">${rt.get("nick")}</span>`;
    _vcard.innerHTML = `${_img}
            <div class="vh" rootid=${rt.get("rid") || rt.id}>
                <div class="vhead">${_nick} ${uaMeta}</div>
                <div class="vmeta">
                    <span class="vtime">${timeAgo(
                      rt.get("insertedAt") || rt.createdAt,
                      root.locale
                    )}</span>
  ...
  let commitEvt = () => {
    ...
    let comment = new Ct();
    let __path =
      "*" === _path ? location.pathname.replace(/index\.html?$/, "") : _path;
    ...
    if (atData["rid"]) {
      ...
      defaultComment["comment"] = defaultComment["comment"].replace(
        "<p>",
        `<p><a class="at" href="#${pid}">${atData["at"]}</a> , `
      );
    }
    ...
    comment
      ...
      .then((ret) => {
        ...
        try {
          if (atData["rid"]) {
            let vquote =
              Utils.find(root.el, '.vquote[rid="' + atData["rid"] + '"]') ||
              createVquote(atData["rid"]);
            ...
            if (_count) {
              ...
              Utils.find(root.el, ".vcount").innerHTML =
                '<span class="num">1</span> ' + root.locale["tips"]["comments"];
            }
